/*When you press the User push button the LEDs should flash faster for 10 seconds 
then return to normal. (press and release)*/
#include <stdio.h>
#include <string.h>
...
/* Private variables ---------------------------------------------------------*/
const Led_TypeDef LEDs[] = {LED3, LED4, LED6, LED8, LED10, LED9, LED7, LED5};
...
int main(int argc, char **argv)
{
  ...
  if(gyroRc != GYRO_OK) {
    ...
    switch(currentState)
    {
    	//case 0 slow blinking, switch not pressed
    	case 0:
    		if (myTickCount ==500)				//creating 1 sec delay using myTickCount==1000
    		{
    			for(int j= 0; j < numLEDs ; j++) 		//switching led using for loop
    			BSP_LED_Toggle(LEDs[j]);		//toggling led
    			myTickCount = 0;
    		}
			if(BSP_PB_GetState(BUTTON_USER))	//getting user switch inpu
    				currentState = 1;

    	break;

    	//case 1 fast blinking for 10 sec after switch presses
    	case 1:
    	for(int j= 0; j < numLEDs ; j++) 		//switching led using for loop
    		BSP_LED_On(LEDs[j]);
    		j=0;
